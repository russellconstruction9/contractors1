...
interface AddPunchListPhotoModalProps {
  isOpen: boolean;
  onClose: () => void;
  projectId: number;
...
const AddPunchListPhotoModal: React.FC<AddPunchListPhotoModalProps> = ({ isOpen, onClose, projectId, punchListItemId }) => {
    const { addPhotoToPunchListItem } = useData();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        await addPhotoToPunchListItem(projectId, punchListItemId, imagePreviews, description || "Punch list photo");
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Add Photo to Punch List Item">
            <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                 ...
                 <div>
                    <label htmlFor="description" className="block text-sm font-medium text-gray-700">Optional Description</label>
                    <textarea
                        id="description"
                        value={description}
                        ...
                        placeholder="e.g., 'Crack in the drywall near outlet.'"
                    />
                </div>
                <div className="flex justify-end space-x-3 pt-4">
                    <Button type="button" variant="secondary" onClick={onClose}>Cancel</Button>
                    <Button type="submit" disabled={imagePreviews.length === 0}>Add to Item</Button>
                </div>
            </form>
        </Modal>
    );
...
export default AddPunchListPhotoModal;
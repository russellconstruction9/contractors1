...
const AddProjectModal: React.FC<AddProjectModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name || !address || !startDate || !endDate || !budget || !markupPercent) {
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Add New Project">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                <label htmlFor="projectMarkup" className="block text-sm font-medium text-gray-700">Billing Markup (%)</label>
                <input type="number" id="projectMarkup" value={markupPercent} onChange={e => setMarkupPercent(e.target.value)} className="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" placeholder="e.g., 20" min="0" required />
            </div>
        </div>
         <div>
